...
Step4.1 -> If left half is sorted then, check if complete array is sorted by checking nums[left] <= nums[mid] <= nums[right]. If so, go towards left, right = mid-1 else go right(is unsorted) right = mid + 1
Step4.2 -> Else check if left to right is sorted => right = mid - 1 ;; else, left = mid + 1
